...
#########################################################################################
	...
	def load_child(self, pollutant, year, sourceFileInfo):
		"""
		Gets the map information from the visualization
		Loads a layer onto the base map for the visualization

		Args:
			pollutant: The pollutant that was selected
			year: The year that was selected
			sourceFileInfo: The information on the source file, saved as a dict.

		Returns:
			None
			
		"""
		self.image, self.min, self.max, self.caption = self.load_data_child(pollutant, year, sourceFileInfo)
	...
	@st.cache
	def load_data_child(self, pollutant, year, sourceFile):
		"""
		Chooses a way to load the child based on the file type.
		...
		if (sourceFile["fileType"] == "zarr"):
			return self.load_zarr_child(pollutant, year, sourceFile["fileName"])
		if (sourceFile["fileType"] == "shp"):
			return self.load_shp_child(pollutant, year, sourceFile["fileName"])
		else: #Do more later.
			return None

	@st.cache
	def load_zarr_child(self, pollutant, year, sourceFileInfo):
		"""
		Loads a zarr to be used as a child.
		...
		Returns:
			A color map, the min of the data, the max of the data, and the caption.
			
		"""
		ds = xr.open_zarr(sourceFileInfo) #how to get file name?
		caption = f'{ds[pollutant].standard_name} ({ds[pollutant].units})'
		data = ds[pollutant].loc[f"{year}-01-01"].data
...
###########################################################################################
	...
	@st.cache
	def load_chloropleth(self, gdfFilename, dfFilename, pollutant, year):
		"""
		Loads a zarr to be used as a child.

		Args:
			gdfFilename: The name of the gdf to be loaded. See caces.py
			dfFilename: The name of the df to be loaded. See caces.py
			pollutant: The pollutant that was selected
			year: The year that was selected
		...
		"""
		df, gdf = self.load_shp_child(gdfFilename, dfFilename, pollutant, year)
		...
		folium.Choropleth(
			...
			fill_opacity=0.7,
			line_opacity=0.2,
			legend_name=CAPTION[pollutant],
		).add_to(m)
		# call to render Folium map in Streamlit
		folium_static(m, width=700, height=800)

	@st.cache
	def load_shp_child(self, gdfFilename, dfFilename, year, pollutant):
		"""
		Reads the file from the gdf
		...
		df = df[(df.year==year) & (df.pollutant==pollutant)][['fips','pred_wght']].astype({'fips':'str'})	
